...
const SideBar = () => {
  const path = usePathname();
  console.log("Current Path:", path); // Debugging the current path

  const normalizePath = (str: string): string => {
    return decodeURIComponent(str)
      .toLowerCase()
      .replace(/\s+/g, "") // Remove spaces without adding dashes
      .replace(/^\/+|\/+$/g, ""); // Remove leading/trailing slashes
  };

  const isActive = (link: NavLinkType): boolean => {
    const normalizedPath = normalizePath(path); // Normalize current path
    const normalizedLinkName = normalizePath(link.name); // Normalize link name

    console.log("Normalized Path:", normalizedPath); // Debugging the normalized path
    console.log("Normalized Link Name:", normalizedLinkName); // Debugging the normalized link

    // Special case for Overview link where "/" needs to match
    if (normalizedPath === "" && link.name === "Overview") return true;

    // Check if the current path matches exactly or starts with the link's normalized path
    return normalizedPath === normalizedLinkName || normalizedPath.startsWith(normalizedLinkName + "/");
  };
