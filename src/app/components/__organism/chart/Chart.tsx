...
const Chart = ({ data }: ChartDataPropsType) => {
  return (
    <div className="bg-blue-200 grid grid-cols-1 md:grid-cols-[65.93%_27.74%] justify-between py-[31px]">
      {/* Donut Pie Chart Container */}
      <div className="h-[240px] w-full flex justify-center items-center bg-yellow-200 aspect-video relative">
        <PieChart
          series={[
            {
              data: data.map((item) => ({
                ...item,
                label: undefined, // Hide labels in the chart
              })),
              innerRadius: 60, // Larger inner radius for a bigger hole (donut effect)
              outerRadius: 100, // Ensure outer radius is large enough
            },
          ]}
          width={240}
          height={240}
...
const Label = ({
  ...
  return (
    <div className="flex flex-row gap-4">
      <div
        className={`w-[5px] bg-[${color ?? "#000000"}] h-full min-h-[43px] rounded-sm`}
      ></div>
      {/* Default color black if not provided */}
      <div className="flex flex-col items-start justify-center gap-1">
        <p className="text-[12px] font-normal text-[#696868]">{label}</p>
